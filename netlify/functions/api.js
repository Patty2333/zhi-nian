/**
 * 织念 ZhiNian - Netlify Function API
 * 功能：用户注册/登录、卡片数据云端同步
 */
  ...
  try {
    ...
    if (path === 'register' && event.httpMethod === 'POST') {
      ...
      if (!username || !password || username.length < 2 || password.length < 4) {
        return { statusCode: 400, headers, body: JSON.stringify({ error: '用户名至少2位，密码至少4位' }) }
      }
      if (usersStore[username]) {
        return { statusCode: 409, headers, body: JSON.stringify({ error: '用户名已存在' }) }
      }
      const salt = makeSalt()
      ...
      return { statusCode: 200, headers, body: JSON.stringify({ ok: true, token, userId, username }) }
    }
    ...
    if (path === 'login' && event.httpMethod === 'POST') {
      ...
      return { statusCode: 200, headers, body: JSON.stringify({ ok: true, token, userId: user.userId, username }) }
    }

    // POST sync
    if (path === 'sync' && event.httpMethod === 'POST') {
      const token = (event.headers.authorization || event.headers['x-token'] || '').replace('Bearer ', '')
      const userId = activeTokens.get(token)
      if (!userId) return { statusCode: 401, headers, body: JSON.stringify({ error: '未登录' }) }
      dataStore[userId] = { cards: body.cards || [], links: body.links || [] }
      return { statusCode: 200, headers, body: JSON.stringify({ ok: true, message: '同步成功' }) }
    }
    ...
    if (path === 'data' && event.httpMethod === 'GET') {
      ...
      const userId = activeTokens.get(token)
      if (!userId) return { statusCode: 401, headers, body: JSON.stringify({ error: '未登录' }) }
      return { statusCode: 200, headers, body: JSON.stringify(dataStore[userId] || { cards: [], links: [] }) }
    }
